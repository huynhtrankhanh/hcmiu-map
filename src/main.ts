...
const withBottomRightLift = (construction: Element) =>
  ...
  const classroom = (name: string) => h("div.w-20.h-16.border.border-black")
const rowOfClassrooms = () => h("div.grid.grid-cols-6.grid-rows-2", Array.from({ length: 12 }, () => classroom("A6.111")))

const root = document.querySelector<HTMLDivElement>("#app")!;
root.appendChild(
  h("div.grid.grid-cols-5.grid-rows-5.w-fit", {style:"grid-template-rows:repeat(5,auto);grid-template-columns:repeat(5,auto)"}, [
    [YES("STAIRS"), NO(), YES("STAIRS"), NO(), NO()],
    [YES(withTopRightLift(twoDoors("A9.100"))), rowOfClassrooms(), YES(), YES(), YES()],
    [YES(), NO(), NO(), NO(), NO()],
    [
